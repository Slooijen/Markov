...
public class MarkovDecisionProblem
{
	...
	public void setpPerform(double pPerform)
	{
		this.pPerform = pPerform;
	}
	...
	public void setpSidestep(double pSidestep)
	{
		this.pSidestep = pSidestep;
	}
	...
	public void setpBackstep(double pBackstep)
	{
		this.pBackstep = pBackstep;
	}
	...
	public double performAction(Action action)
	{
		...
		else
		{
			...
			if (prob < pPerform)
				...
				if (prob < pPerform + pSidestep / 2)
					doAction(Action.previousAction(action));
				else
					if (prob < pPerform + pSidestep)
						doAction(Action.nextAction(action));
					else
						if (prob < pPerform + pSidestep + pBackstep)
	 ...
	 *            as DOWN)
	 * @param pNoStep,
	 *            the probability an action is not performed at all (e.g. UP is
	 *            not executed)
	 */
